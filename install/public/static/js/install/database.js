define(["jquery", 'jquery-ui', "text!templates/database.html"], function($, ui,
    tpl) {
    ...
    Database.prototype.init = function() {
            ...
            this.$nextBtn = $(this.$container).find(
                '.modal-footer > button');
            ...
            this.$form.submit(function() {
                if ($this.platform == 'default') {
                    //do somecheck here
                    if ($this.$server.val() == '') {
                        $this.$server.focus().parent().effect(
                            'shake');
                        return false;
                    }
                    if ($this.$db.val() == '') {
                        $this.$db.focus().parent().effect(
                            'shake');
                        return false;
                    }
                    if ($this.$user.val() == '') {
                        $this.$user.focus().parent().effect(
                            'shake');
                        return false;
                    }
                }
    ...
    Database.prototype.show = function() {
            ...
            if (this.platform == 'sae') {
                ...
                this.$nextBtn.before(
                    '<p class="alert alert-error pull-left"><strong>SAE平台无需填写表单，直接点击导入即可.</strong></p>'
                );
